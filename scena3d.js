...
function box(x, y, z, width, depth, height, material = 0x87929d) {
  const mesh = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: material, wireframe: wire })
  );
...
function addRoofBeam(x, z1, y1, z2, y2) {
  ...
  const mesh = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x9ca6af, wireframe: wire })
  );
...
export function aggiornaScena3D(s) {
  ...
  const W = Number(s.generale.luce);
  const H = Number(s.generale.altezzaPilastro);
  const pendenza = Number(s.generale.pendenzaCopertura);
  const basePilastro = Number(s.pilastri.base || 40) / 100;
  const profonditaPilastro = Number(s.pilastri.altezzaSezione || 60) / 100;
  const rise = W * pendenza / 200;
  const ridgeY = H + rise;

  // X = lunghezza, Y = altezza, Z = profondita.
  const xs = [0];
  let x = 0;
  s.campate.forEach(campata => {
    x += Number(campata.interasse);
    xs.push(x);
  });

  // Pilastri verticali: da Y=0 a Y=H su entrambe le file.
  xs.forEach(px => {
    box(px, H / 2, 0, basePilastro, profonditaPilastro, H);
    box(px, H / 2, W, basePilastro, profonditaPilastro, H);
  });

  // Travi di banchina con quota superiore Y=H.
  const traveH = 0.6;
  box(L / 2, H - traveH / 2, 0, L, 0.5, traveH);
  box(L / 2, H - traveH / 2, W, L, 0.5, traveH);

  // Travi trasversali inclinate nel piano YZ.
  xs.slice(0, -1).forEach((px, index) => {
    const span = Number(s.campate[index].interasse);
  ...
  // Pavimento sul piano XZ a Y=0.
  box(L / 2, -0.1, W / 2, L, 0.2, W, envelopeMaterial);
  fit(s);
}
...
function fit(s) {
  ...
  const W = Number(s.generale.luce);
  const H = Number(s.generale.altezzaPilastro);
  camera.position.set(L * 0.95, H * 1.1, -W * 1.25);
...
export function vistaIsometrica() {
  ...
  const W = Number(s.generale.luce);
  const H = Number(s.generale.altezzaPilastro);
  camera.position.set(L * 0.9, H, -W * 1.1);
  controls.target.set(L / 2, H / 2, W / 2);
  controls.update();
}
