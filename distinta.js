// distinta.js — Generazione distinta + prezzi + export CSV

import { calcolaDerivati, lunghezzaTotale } from './modello.js';
import { trovaTrave, trovaTegolo, getPrezzoM3, getPrezzoMl } from './catalogo.js';

const DENSITA_CLS = 2.5;

/**
 * Costruisce la distinta completa dallo stato.
 */
export function costruisciDistinta(stato) {
  const d = calcolaDerivati(stato);
  const tra = trovaTrave(stato.travi.tipoBanchina) || { base: 40, altezza: 60, id: 'TL' };
  ...
  righe.push({
    ...
    tipo: tra.id || 'TL',
    quantita: stato.campate.length * 2,
  ...
  // 6. Coppelle (metro lineare)
  const numCop = Math.ceil(d.numTegoli * 1.1);
  righe.push({
    famiglia: 'Copertura',
    elemento: 'Coppelle',
    tipo: stato.copertura.coppellaId || 'CC332',
    quantita: numCop,
    dimensioni: '30×15 cm',
    volume: 0,
    peso: numCop * 0.05,
...
/**
 * Aggiorna la tabella HTML della distinta.
 */
