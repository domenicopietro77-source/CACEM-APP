...
/**
 * Carica l'archivio commesse da localStorage.
 */
export function caricaCommesse() {
  ...
  try {
    ...
    if (!dati || !Array.isArray(dati.commesse)) {
      return { commesse: [], commessaCorrente: null };
    }
    return dati;
  } catch {
...
/**
 * Salva l'archivio commesse su localStorage.
 */
...
/**
 * Ritorna la lista di tutte le commesse.
 */
...
/**
 * Crea una nuova commessa con nome dato.
 * Lo stato parte da statoDefault() — ma il chiamante lo passerà.
 */
...
export function eliminaCommessa(id) {
  ...
  if (archivio.commessaCorrente === id) {
    archivio.commessaCorrente = archivio.commesse.length > 0
      ? archivio.commesse[0].id
      : null;
  }
...
/**
 * Duplica una commessa esistente.
 */
...
/**
 * Imposta una commessa come corrente.
 */
...
/**
 * Aggiorna lo stato della commessa corrente.
 */
...
/**
 * Esporta una commessa come file JSON scaricabile.
 */
...
/**
 * Importa una commessa da file JSON.
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      try {
        ...
        if (idx >= 0) {
          archivio.commesse[idx] = c;
        } else {
          archivio.commesse.push(c);
        }
